...
<<<<<<< HEAD
    ...
    //add weixing [statistics build v1]20170406:b
    int ObMergeServerService::ms_fetch_collection_list(const int64_t receive_time,
        const int32_t version,
        const int32_t channel_id,
        onev_request_e*req,
        common::ObDataBuffer &in_buffer,
        ...
        const int64_t timeout_us)
    {
      ...
      UNUSED(in_buffer);
      UNUSED(timeout_us);
      const int32_t MY_FETCH_VERSION =1;
      ObResultCode rc;
      int64_t pos = 0;
      ObArray<uint64_t> list_array;
      char query_collection_list_buf[1024];
      ObString query_collection_list;
      ObSQLResultSet collection_list;
      ObSQLSessionInfo session;
      ObSqlContext context;
      int64_t schema_version = 0;
      int32_t &err = rc.result_code_;
      ObResultSet result;
      databuff_printf(query_collection_list_buf, 1024, pos, "SELECT table_id, column_id FROM __all_udi_monitor_list");
      if(pos > 1023)
      {
        err = OB_BUF_NOT_ENOUGH;
        TBSYS_LOG(WARN, "query collection list overflow, err = %d", err);
      }
      else
      {
        query_collection_list.assign_ptr(query_collection_list_buf, static_cast<ObString::obstr_size_t>(pos));
        if (OB_SUCCESS != (err = sql_proxy_.init_sql_env(context, schema_version, collection_list, session)))
        {
          TBSYS_LOG(WARN, "init sql env error, err=%d", err);
        }
        else
        {
          if(OB_SUCCESS != (err = sql_proxy_.execute(query_collection_list, collection_list, context, schema_version)))
          {
            TBSYS_LOG(WARN,"execute privilge sql failed,sql=%.*s, err = %d", query_collection_list.length(), query_collection_list.ptr(), err);
          }
          else if(OB_SUCCESS != (err = collection_list.get_result_set().open()))
          {
            TBSYS_LOG(ERROR, "open collection list failed, err = %d", err);
          }
          common::ObResultCode result_msg;
          result_msg.result_code_ = err;
          if(OB_SUCCESS != (err = result_msg.serialize(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position())))
          {
            TBSYS_LOG(WARN, "failed to encode result buffer, err[%d]", err);
          }
          else
          {
            ...
            {
              if(OB_SUCCESS != (err = collection_list.get_result_set().get_next_row(row)))
              {
                if(OB_ITER_END == err)
                {
                  TBSYS_LOG(INFO,"has itered to the end!, err=%d",err);
                  err = OB_SUCCESS;
                }
                else
                {
                  TBSYS_LOG(ERROR, "failed to get next row, err=%d", err);
                }
                break;
              }
              else if(row != NULL)
              {
                TBSYS_LOG(INFO, "TEST::WEIXING row=[%s]", to_cstring(*row));
                uint64_t table_id = OB_INVALID_ID;
                uint64_t column_id = OB_INVALID_ID;
                const common::ObObj *cell1 = NULL;
                const common::ObObj *cell2 = NULL;
                //table_id
                if(OB_SUCCESS != (err=row->get_row_desc()->get_tid_cid(0,table_id,column_id)))
                {
                  TBSYS_LOG(ERROR, "failed to get tid and cid, err=%d",err);
                  break;
                }
                if(OB_SUCCESS != (err=row->get_cell(table_id,column_id,cell1)))
                {
                  TBSYS_LOG(ERROR, "failed to get cell, err=%d",err);
                  break;
                }
                else
                {
                  int64_t tmp_tid = 0;
                  cell1->get_int(tmp_tid);
                  uint64_t tid = static_cast<uint64_t>(tmp_tid);
                  TBSYS_LOG(INFO,"TEST::WEIXING the tid is %ld",tid);
                  list_array.push_back(tid);
                }
                //column_id
                if(OB_SUCCESS != (err=row->get_row_desc()->get_tid_cid(1,table_id,column_id)))
                {
                  TBSYS_LOG(ERROR, "failed to get tid and cid, err=%d", err);
                  break;
                }
                if(OB_SUCCESS != (err=row->get_cell(table_id,column_id,cell2)))
                {
                  TBSYS_LOG(ERROR, "failed to get cell, err=%d", err);
                  break;
                }
                ...
                {
                  int64_t tmp_cid = 0;
                  cell2->get_int(tmp_cid);
                  uint64_t cid = static_cast<uint64_t>(tmp_cid);
                  TBSYS_LOG(INFO,"TEST::WEIXING the cid is %ld",cid);
                  list_array.push_back(cid);
                }
              }
              else
              {
                TBSYS_LOG(INFO,"TEST::WEIXING it is null!");
              }
            }
            if(OB_SUCCESS != (err = serialization::encode_vi64(out_buffer.get_data(),out_buffer.get_capacity(),out_buffer.get_position(),list_array.count())))
            {
              TBSYS_LOG(WARN,"failed to serialize the list size, err=%ld",list_array.count());
            }
            else
            {
              for(int64_t i = 0; i < list_array.count(); i++)
              {
                if(OB_SUCCESS != (err = serialization::encode_vi64(out_buffer.get_data(),out_buffer.get_capacity(),out_buffer.get_position(),static_cast<int64_t>(list_array.at(i)))))
                {
                  TBSYS_LOG(WARN,"failed to serialize the list id=%ld, err=%d",list_array.at(i),err);
                }
              }
            }
          }
      ...
      context.schema_manager_ = schema_mgr_->get_user_schema(schema_version);
      //int64_t j =0;
      for(int64_t j = 0; j < list_array.count();j = j+2)
      {
        pos = 0;
        char del_stat_buff[512];
        ObString del_stat;
        int64_t tmp = j;
        tmp++;
        databuff_printf(del_stat_buff,512,pos,"DELETE FROM __all_statistic_info WHERE table_id = %ld AND column_id = %ld",list_array.at(j),list_array.at(j+1));
        if(pos > 511)
        {
          err = OB_BUF_NOT_ENOUGH;
          TBSYS_LOG(WARN, "del_stat buff overflow! err = %d",err);
        }
        else
        {
          context.disable_privilege_check_ = true;
          context.session_info_->set_current_result_set(&result);
          del_stat.assign_ptr(del_stat_buff, static_cast<ObString::obstr_size_t>(pos));
          if(OB_SUCCESS != (err = result.init()))
          {
            TBSYS_LOG(WARN,"init result set failed,%d",err);
          }
          else if(OB_SUCCESS != (err = ObSql::direct_execute(del_stat,result,context)))
          {
            TBSYS_LOG(WARN, "direct execute sql =%.*s failed, ret=%d",del_stat.length(),del_stat.ptr(),err);
          }
          else if(OB_SUCCESS != (err = result.open()))
          {
            TBSYS_LOG(WARN, "open result failed, ret=%d",err);
          }
          else
          {
            OB_ASSERT(result.is_with_rows() == false);
            TBSYS_LOG(INFO, "execute %.*s success", del_stat.length(),del_stat.ptr());
            int tmp_err = result.close();
            if (OB_SUCCESS != tmp_err)
            {
              TBSYS_LOG(WARN, "failed to close result set, ret=%d", tmp_err);
            }
            result.reset();
          }
        }
      }
      if (OB_SUCCESS != (err = merge_server_->send_response(OB_FETCH_COLLECTION_LIST_RESPONSE,
                                         MY_FETCH_VERSION,out_buffer, req, channel_id)))
      {
        TBSYS_LOG(WARN, "send response error, err:[%d]",err);
      }
      return err;
    }
    //add e
