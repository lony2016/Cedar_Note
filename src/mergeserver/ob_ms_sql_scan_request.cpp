...
namespace oceanbase
{
  namespace mergeserver
  {
    ...
    int ObMsSqlScanRequest::get_next_row(oceanbase::common::ObRow &row)
    {
        ...
        else if (!this->is_end_ && this ->is_finish())
        {
            if ((total_sub_request_count_ - finished_sub_request_count_ < max_parallel_count_)
                    && (merger_operator_.get_mem_size_used() < max_cs_result_mem_size_))
            {
                TBSYS_LOG(DEBUG, "wanglei::total_sub_request_count_=%d, finished_sub_request_count_=%d, max_parallel_count_=%ld,"
                ...
                {
                    bool has_next = false;
                    if ( this ->rpc_scan_->get_sql_read_trategy()->has_next_range() )
                    {
                        ObNewRange query_range;
                        if ( (err = this ->rpc_scan_->get_sql_read_trategy()->get_next_scan_range(query_range, has_next)) != OB_SUCCESS)
                        {
                            err = OB_ERR_UNEXPECTED;
                            TBSYS_LOG(WARN, "fail to get next range %d",err);
                        }
                        else if ((err = this ->rpc_scan_->init_next_scan_param(query_range)) != OB_SUCCESS)
                        {
                            err = OB_ERR_UNEXPECTED;
    ...
    //add wanglei [semi join in expr] 20161131:b
    int ObMsSqlScanRequest::init_ex(const uint64_t count, const uint32_t mod_id,  sql::ObRpcScan *rpc_scan, common::ObSessionManager * session_mgr )//slwang note:最后两个参数默认值为空
    {
        int err = OB_SUCCESS;
    ...
    int ObMsSqlScanRequest::set_request_param_only(ObSqlScanParam &scan_param,const common::ObRpcScanHint & hint)
    {
        UNUSED(hint);
        int err = OB_SUCCESS;
        int64_t sharding_limit_offset = 0;
    ...
    int ObMsSqlScanRequest::get_next_row_with_one_scan_request(oceanbase::common::ObRow &row,
                                           ...
                                           )
    {
        ...
        else if (!this->is_end_ && this ->is_finish())
        {
                    ...
                    && (merger_operator_.get_mem_size_used() < max_cs_result_mem_size_))
            {
                ...
                {
                    bool has_next = false;
                    if ( get_sql_read_trategy->has_next_range() )
                    {
                        ObNewRange query_range;
                        if ( (err = get_sql_read_trategy->get_next_scan_range(query_range, has_next)) != OB_SUCCESS)
                        {
                            err = OB_ERR_UNEXPECTED;
                            TBSYS_LOG(WARN, "fail to get next range %d",err);
                        }
                        else if ((err = rpc_scan_->init_next_scan_param_with_one_request(query_range,scan_param_,this)) != OB_SUCCESS)
                        {
                            err = OB_ERR_UNEXPECTED;
                            TBSYS_LOG(WARN, "fail to init  next scan range  param %d",err);
                        }
