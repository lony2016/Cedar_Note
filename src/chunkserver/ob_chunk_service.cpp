...
<<<<<<< HEAD
//add weixing [statistics build] 20170105:b
...
<<<<<<< HEAD
      ...
      se_index_task_(this),
      //add e
      fetch_ups_task_(this), report_tablet_task_(this)//add weixing [statistics build] 20170207:b
      ,statistics_task_(this),statistics_check_task_(this)
    //add e
=======
...
<<<<<<< HEAD
      ...
      if(OB_SUCCESS == rc)
      {
        ...
        //add by weixing [statistics build]20170206:b
        if(OB_SUCCESS != (rc = chunk_server_->get_tablet_manager().start_statistics_collector()))
        {
          TBSYS_LOG(ERROR, "Fail to start statistics collector! ret = %d", rc);
        }
        //add e
      }
      //add e
...
<<<<<<< HEAD
          ...
          case OB_START_GATHER_OPERATION:
            rc = cs_start_gather_operation(version,channel_id, req, in_buffer, out_buffer);
            TBSYS_LOG(INFO,"TEST::WEIXING receive successfully!");
            break;
          //add e
=======
>>>>>>> refs/remotes/origin/master
...
<<<<<<< HEAD
    //add weixing [statistics build v1]20170331:b
    int ObChunkService::cs_start_gather_operation(const int32_t version,
        const int32_t channel_id,
        onev_request_e*req,
        ObDataBuffer &in_buf,
        ObDataBuffer &out_buf)
    {
      int ret = OB_SUCCESS;
      ...
      int64_t wait_time = 5000;
      if(OB_SUCCESS != (ret = serialization::decode_i64(in_buf.get_data(),in_buf.get_capacity(),in_buf.get_position(),&count)))
      {
        TBSYS_LOG(WARN,"deserialize count error, err=%d",ret);
      }
      else
      {
        for(int64_t i = 0; i < count; i=i+2)
        {
          uint64_t tmp_tid;
          uint64_t tmp_cid;
          if(OB_SUCCESS != (ret = serialization::decode_i64(in_buf.get_data(),in_buf.get_capacity(),in_buf.get_position(),(int64_t*)&tmp_tid)))
          {
            TBSYS_LOG(WARN,"deserialize tmp_tid error, err=%d",ret);
          }
          else if(OB_SUCCESS != (ret = serialization::decode_i64(in_buf.get_data(),in_buf.get_capacity(),in_buf.get_position(),(int64_t*)&tmp_cid)))
          {
            TBSYS_LOG(WARN,"deserialize tmp_cid error, err=%d",ret);
          }
          else
          {
            ObGatherTableInfo gather_info;
            gather_info.table_id_ = tmp_tid;
            gather_info.add_column_id(tmp_cid);
            TBSYS_LOG(INFO,"the push tid is %ld, cid is %ld", tmp_tid,tmp_cid);
            if(OB_SUCCESS != (ret = statistics_task_.set_schedule_info(gather_info)))
            {
              TBSYS_LOG(WARN,"failed to push gather info into list! err=%d", ret);
            }
          }
        }
      }
      result_msg.result_code_ = ret;
      if(!statistics_task_.get_collection_tag())
      {
        statistics_task_.set_collection_tag(true);
        if(chunk_server_->get_tablet_manager().get_chunk_merge().is_merge_reported() && statistics_task_.get_schedule_task_size()>0)
        {
          if(OB_SUCCESS != (ret = timer_.schedule(statistics_task_, wait_time, false)))
          {
            TBSYS_LOG(WARN, "can't schedule statistics_task_ after wait %ld us", wait_time);
          }
        }
        else
        {
          if(!chunk_server_->get_tablet_manager().get_chunk_merge().is_merge_reported())
          {
            TBSYS_LOG(WARN,"Chunkserver is merging! Abort this operation!");
          }
          else if(statistics_task_.get_schedule_task_size() == 0)
          {
            TBSYS_LOG(WARN,"there is no gather info!");
          }
        }
      }
      else
      {
        TBSYS_LOG(WARN,"Chunkserver is gathering statistics! Abort this operation");
      }
      return ret;
    }
    //add e
...
<<<<<<< HEAD
    ...
    void ObChunkService::SeIndexTask::runTimerTask()
    {
     ...
     void ObChunkService::StatisticsTask::runTimerTask()
     {
       int err = OB_SUCCESS;
       ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
       unset_scheduled();
       TBSYS_LOG(INFO,"weixing I want to start round to build index!");
       if(tablet_manager.get_statistics_collector().can_launch_next_round())
       {
         TBSYS_LOG(INFO,"weixing I can launch next round!");
         err = tablet_manager.build_statistics();
         if(OB_SUCCESS != err)
         {
           TBSYS_LOG(INFO,"weixing wait for next round!");
         }
       }
     }
     void ObChunkService::StatisticsTask::set_collection_tag(bool tag)
     {
       ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
       tablet_manager.get_statistics_collector().set_collection_tag(tag);
     }
     bool ObChunkService::StatisticsTask::get_collection_tag()
     {
       ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
       return tablet_manager.get_statistics_collector().get_collection_tag();
     }
     int ObChunkService::StatisticsTask::set_schedule_info(common::ObGatherTableInfo gather_info)
     {
       ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
       TBSYS_LOG(INFO,"TEST::WEIXING try to dump gather_info");
       gather_info.dump();
       return tablet_manager.get_statistics_collector().add_collection_task(gather_info);
     }
     int64_t ObChunkService::StatisticsTask::get_schedule_task_size()
     {
       ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
       return tablet_manager.get_statistics_collector().get_collection_task_size();
     }
     void ObChunkService::StatisticsCheckTask::runTimerTask()
     {
       ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
       tablet_manager.get_statistics_collector().check_self();
     }
     //add e
