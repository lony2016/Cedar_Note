...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObTabletManager::start_statistics_collector()
    {
      return statistics_collector_.init(this);
    }
    int ObTabletManager::build_statistics()
    {
      int ret = OB_SUCCESS;
      TBSYS_LOG(INFO,"weixing start to build statistics");
      if(OB_SUCCESS == ret)
      {
        if(OB_SUCCESS != (ret = statistics_collector_.schedule()))
        {
          TBSYS_LOG(ERROR,"failed to start statistics collector schedule!");
        }
      }
      return ret;
    }
