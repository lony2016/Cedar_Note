...
namespace oceanbase
{
  namespace common
  {
    ...
    bool is_numerical_type(const oceanbase::common::ObObj &value)
    {
      bool flag = false;
      ObObjType type = value.get_type();
      if (type == ObIntType ||
          type == ObFloatType ||
          type == ObDoubleType ||
          type == ObDecimalType ||
...
//          ||type == ObInt32Type
    ...
    double calc_proportion(oceanbase::common::ObExprObj &value,
                           oceanbase::common::ObExprObj &max,
                           oceanbase::common::ObExprObj &min)
    {
      double proportion = 0.5;
      /*
      int64_t t_value = 0, t_max = 0, t_min = 0;
      float f_value = 0.0, f_max = 0.0, f_min = 0.0;
      double d_value = 0.0, d_max =0.0 , d_min = 0.0;
      if(value < min)
        return 0.0;
      else if( value > max)
        return 1.0;

      if (value.get_type() == ObIntType)
      {
        value.get_int(t_value);
        max.get_int(t_max);
        min.get_int(t_min);

        proportion = static_cast<double>((t_value - t_min + 1)) / static_cast<double>((t_max - t_min +1));
      }
      else if (value.get_type() == ObFloatType)
      {
        value.get_float(f_value);
        max.get_float(f_max);
        min.get_float(f_min);

        proportion = 1.0 * (f_value - f_min + 1) / (f_max - f_min + 1);
      }
      else if (value.get_type() == ObDoubleType)
      {
        value.get_double(d_value);
        max.get_double(d_max);
        min.get_double(d_min);

        proportion = (d_value - d_min + 1) / (d_max - d_min + 1);
      }
      else if (value.get_type() == ObDecimalType)
      {
...
*/
      ...
      int cmp = 0;
      bool ret = false;
      common::ObExprObj tmp1;
      common::ObExprObj tmp2;
      common::ObExprObj result;

      common::ObObj obj_min;
      common::ObExprObj promoted_min;
      common::ObObj obj_max;
      common::ObExprObj promoted_max;
      common::ObObj obj_value;
      common::ObExprObj promoted_value;

      //handle max min is null
      if (min.get_type() == ObNullType &&
          max.get_type() == ObNullType)
      {
        if (value.get_type() == ObNullType)
        {
          proportion = 1.0;
        }
        else
      ...
      else if (min.get_type() == ObNullType ||
               max.get_type() == ObNullType)
      {
        TBSYS_LOG(WARN,"min or max is null cause can't compute proportion.");
        ret = true;
      }
      else if (min.get_type() != max.get_type())
      {
        TBSYS_LOG(WARN,"min and max is not same type.");
      }
      ...
      else
      {
        ...
        {
          value.compare(min,cmp);
          if (cmp <= 0) //we will loss a lot precision if cmp == 0
          {
            proportion = 0.0;
          }
          else
          {
            value.compare(max,cmp);
            if (cmp >= 0)
            {
              proportion = 1.0;
            }
            ...
            {
              //special processing int and datatime type
              double v_min;
              double v_max;
              double v_value;
              bool flag = false;
...
//              case ObIntervalType:
              ...
              case ObModifyTimeType:
              {
                ...
                int64_t time = 0;
                //min
                obj.get_timestamp(time);
                v_min = static_cast<double> (time);
                //max
                max.to(obj);
                obj.get_timestamp(time);
                v_max = static_cast<double> (time);
                //value
                value.to(obj);
                ObObj expected_type;
                ObObj casted_obj;
                const ObObj * res_obj = NULL;
                expected_type.set_type(min.get_type());
                if (OB_SUCCESS != obj_cast(obj,expected_type,casted_obj,res_obj))
                {
                  TBSYS_LOG(WARN,"value obj cast fail.");
                }
                else if (res_obj != NULL)
                {
                  res_obj->get_timestamp(time);
                  v_value =  static_cast<double> (time);
                  flag = true;
                }
                break;
              }
              case ObIntType:
              {
                v_min = static_cast<double> (min.get_int());
                v_max = static_cast<double> (max.get_int());
...
//              case ObInt32Type:
//              {
//                v_min = static_cast<double> (min.get_int32());
//                v_max = static_cast<double> (max.get_int32());
...
//              }
              ...
              if (flag)
              {
                obj_min.set_double(v_min);
                promoted_min.assign(obj_min);

                obj_max.set_double(v_max);
                promoted_max.assign(obj_max);

                obj_value.set_double(v_value);
                promoted_value.assign(obj_value);

                promoted_value.sub(promoted_min,tmp1);
                promoted_max.sub(promoted_min,tmp2);
                tmp1.div(tmp2,result,true);
              }
              else
              {
                value.sub(min,tmp1);
              ...
              {
              case ObIntType:
                //not should go to there
              {
                int64_t tmp_res = result.get_int();
                proportion = static_cast<double>(tmp_res);
                break;
              }
//              case ObInt32Type: //not should go to there
//              {
//                int tmp_res = result.get_int32();
//                proportion = static_cast<double>(tmp_res);
//                break;
//              }
              case ObFloatType:
              {
                float tmp_res = result.get_float();
                proportion = static_cast<double>(tmp_res);
                break;
              }
              case ObDoubleType:
              {
                double tmp_res = result.get_double();
                proportion = static_cast<double>(tmp_res);
                break;
              }
               //need qx update
...
//                ObDecimal od_result = result.get_decimal();
//                od_result.to_string(buf,MAX_PRINTABLE_SIZE);
//                if (1 != sscanf(buf,"%lf",&proportion))
//                {
//                  proportion = 0.0;
//                  TBSYS_LOG(WARN,"cast to double fail.");
//                }
//                break;
//              }
              default:
      ...
      if (proportion > 1.0)
      {
        TBSYS_LOG(WARN,"compute may be happen error, proportion =%.20lf",proportion);
        proportion = 1.0;
      }
      ...
      TBSYS_LOG(DEBUG,"proportion = %.20lf", proportion);
      return proportion;
    }
