...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObSchemaManagerV2::get_index_columns(uint64_t table_id, uint64_t index_table_id, common::ObArray<uint64_t>& column_array) const
    {
      ...
      {
        uint64_t max_index_column_id = schema->get_max_column_id();
        for(uint64_t loop = OB_APP_MIN_COLUMN_ID;loop < max_index_column_id;loop++)
        {
          if(NULL != (get_column_schema(index_table_id,loop)))
          {
            column_array.push_back(loop);
          }
        }
      }
      TBSYS_LOG(INFO,"index table[%ld] has [%ld] columns",table_id, column_array.count());
      return ret;
    }
    //add :e
        ...
        uint64_t *index_tid_array) const
    {
      ...
      if (is_id_index_hash_map_init_)
      {
        if (hash::HASH_EXIST == id_index_hash_map_.get(tid, il))
        {
          ...
          // add by qx [query optimizer] 20170915 :b
          //query optimizer will get all can used index table id
          if (array_index < OB_MAX_INDEX_NUMS)
          {
              index_tid_array[array_index] = OB_INVALID_ID;
          }
          //add 20170915:e
        }
      }
