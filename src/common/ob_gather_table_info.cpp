  ...
  namespace common
    {
      ...
      {
        TBSYS_LOG(INFO,"======dump gather table info======");
        TBSYS_LOG(INFO,"gather table info dump: table id = %lu",table_id_);
        for(int i = 0; i < columns_list_.get_array_index(); i++)
        {
          TBSYS_LOG(INFO,"gather table info dump: column id = %lu",columns_[i]);
        }
        TBSYS_LOG(INFO,"============ end ===========");
      }
