...
namespace oceanbase
{
  namespace common
  {
    ...
    int BlackList::add_in_black_list(ObNewRange &range, ObServer &server)
    {
      ...
      {
        if (server_[i].get_ipv4() == server.get_ipv4()
            && server_[i].get_port() == server.get_port())
        {
          can_add = true;
          unserved_[i] = 1;
          //index = i;
    ...
    int BlackList::next_replic_server(ObServer &server)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        TBSYS_LOG(WARN, "all replication is failed......DUMP SERVER:\n"
            "count[%d],server:[%s][%s][%s],unserved[%d][%d][%d]", server_count_,
            to_cstring(server_[0]), to_cstring(server_[1]),
            to_cstring(server_[2]), unserved_[0], unserved_[1], unserved_[2]);
      }
      return ret;
    }
    ...
    DEFINE_SERIALIZE(BlackList)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          if (OB_SUCCESS
              != (ret = serialization::encode_i8(buf, buf_len, pos,
                  unserved_[i])))
          {
            break;
          }
    ...
    int BlackListArray::get_next_black_list(BlackList &list)
    {
      int ret = OB_SUCCESS;
    ...
    int BlackListArray::check_range_in_list(ObNewRange &range, bool &in,
        int64_t &index)
    {
      int ret = OB_SUCCESS;
      in = false;
    ...
    int BlackListArray::push(BlackList &list)
    {
      ...
      for (int64_t i = 0; i < list_array_.count(); i++)
      {
        ...
        if (range_itr.table_id_ == range_src.table_id_
            && range_itr == range_src)
        {
          has_new_range = false;
          index = i;
      ...
      {
        if (OB_SUCCESS
            != (ret = range_src.start_key_.deep_copy(range_dst.start_key_,
                arena_)))
        {
          TBSYS_LOG(ERROR, "deep copy start key failed,ret = [%d]", ret);
        }
        else if (OB_SUCCESS
            != (ret = range_src.end_key_.deep_copy(range_dst.end_key_, arena_)))
        {
      ...
      if (OB_SUCCESS == ret && has_new_range)
      {
        ...
        if (OB_SUCCESS
            != (ret = list_to_push.write_list(range_dst, location_list)))
        {
    ...
    int BlackList::set_rowkey_obj_array(char *buf, const int64_t buf_len,
        int64_t &pos, const ObObj *array, const int64_t size)
    {
      int ret = OB_SUCCESS;
    ...
    int BlackList::set_int_obj_value(char *buf, const int64_t buf_len,
        int64_t &pos, const int64_t value)
    {
      int ret = OB_SUCCESS;
      ObObj obj;
      obj.set_int(value);
    ...
    int BlackList::get_rowkey_compatible(const char *buf, const int64_t buf_len,
        int64_t &pos, ObObj *array, int64_t &size)
    {
      ...
      if (OB_SUCCESS == (ret = obj.deserialize(buf, buf_len, pos)))
      {
        if (ObIntType == obj.get_type()
            && (OB_SUCCESS == (ret = obj.get_int(obj_count))))
        {
          // new rowkey format.
