...
int ObRootServer2::start_gather_operation()
{
  ...
  if(this->is_master())
  {
      ...
      {
        TBSYS_LOG(WARN,"generate collection list for statistics! sleep and retry!");
        usleep(3000000);
      }
      if(collection_list.count() > 0)
      {
        TBSYS_LOG(INFO,"TEST::WEIXING the collection list is %ld",collection_list.count());
        if(collection_list.count()%2 == 0)
        {
          ObChunkServerManager::iterator it = this->server_manager_.begin();
          for (; OB_SUCCESS == ret && it != this->server_manager_.end(); ++it)
          {
            if (it->status_ != ObServerStatus::STATUS_DEAD && it->port_cs_ != 0)
            {
              ...
              {
                TBSYS_LOG(WARN, "failed to send signal to cs[%s], err=%d",to_cstring(tmp_server),ret);
              }
            }
          }
        }
        ...
        {
          TBSYS_LOG(ERROR, "the collection task is not entired!, the count=%ld", collection_list.count());
          ret = OB_ERROR;
        }
      }
      else
      {
        TBSYS_LOG(INFO,"there is no collection task to gather!");
      }
  }
  return ret;
}
...
int ObRootServer2::fetch_collection_list(const int64_t version, ObArray<uint64_t> *list)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    TBSYS_LOG(ERROR, "root worker con't be null");
  }
  if(OB_SUCCESS == ret)
  {
    if(OB_SUCCESS != (ret = get_ms(ms)))
    {
      TBSYS_LOG(WARN, "get mergeserver failed, ret = %d", ret);
    }
    else if(OB_SUCCESS != (ret = worker_->get_rpc_stub().get_collection_list_from_ms(ms,config_.safe_wait_init_time,version,list)))
    {
      TBSYS_LOG(WARN, "fetch collection list from ms[%s] failed, version[%ld], ret[%d]", to_cstring(ms),version,ret);
    }
  }
  return ret;
}
