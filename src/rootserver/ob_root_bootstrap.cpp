...
int ObBootstrap::bootstrap_sys_tables(void)
{
  ...
  //add huangcc [statistic information materialization] 20161208:b
  // create table __all_statistic_info
  if (OB_SUCCESS == ret)
  {
    if (OB_SUCCESS != (ret = ObExtraTablesSchema::all_statistic_info(table_schema)))
    {
      TBSYS_LOG(WARN, "failed to get schema of __all_statistic_info, err=%d", ret);
    }
    else if (OB_SUCCESS != (ret = create_sys_table(table_schema)))
    {
      TBSYS_LOG(WARN, "failed to create table for __all_statistic_info, err=%d", ret);
    }
  }
  //add:e

  // create table __all_udi_minyor_list
  if(OB_SUCCESS == ret)
  {
    if(OB_SUCCESS != (ret = ObExtraTablesSchema::all_udi_mointor_list(table_schema)))
    {
      TBSYS_LOG(WARN, "failed to get schema of __all_udi_monitor_list, err=%d", ret);
    }
    else if(OB_SUCCESS != (ret = create_sys_table(table_schema)))
    {
      TBSYS_LOG(WARN, "failed to create table for __all_udi_monitor_list, err=%d", ret);
    }
  }
  //add e
  return ret;
