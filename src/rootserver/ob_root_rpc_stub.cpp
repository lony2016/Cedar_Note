...
int ObRootRpcStub::get_collection_list_from_ms(const ObServer ms, const int64_t timeout, const int64_t data_version, ObArray<uint64_t> *list)
{
  int ret = OB_SUCCESS;
  ObDataBuffer msgbuf;
  ...
  else if(OB_SUCCESS != (ret = client_mgr_->send_request(ms,OB_FETCH_COLLECTION_LIST, DEFAULT_VERSION, timeout, msgbuf)))
  {
  ...
  else
  {
    ...
    int64_t pos = 0;
    ObResultCode result_code;
    //uint64_t all_id = OB_INVALID_ID;
    if(OB_SUCCESS != (ret = result_code.deserialize(msgbuf.get_data(), msgbuf.get_capacity(), pos)))
    {
      TBSYS_LOG(ERROR, "failed to deserialize responser, err=%d", ret);
    }
    ...
    else if(OB_SUCCESS != (serialization::decode_vi64(msgbuf.get_data(), msgbuf.get_capacity(),pos,&size)))
    {
      TBSYS_LOG(WARN, "decode collection list size failed!ret[%d]", ret);
    }
    else
    {
      list->clear();
      ...
      {
        int64_t tmp_id;
        if(OB_SUCCESS != (ret = serialization::decode_vi64(msgbuf.get_data(),msgbuf.get_position(),pos,&tmp_id)))
        {
          TBSYS_LOG(WARN,"failed to decode collection id from msgbuf, ret=%d",ret);
          break;
        }
        else
        {
          TBSYS_LOG(INFO,"TEST::WEIXING the tmp id is %ld",tmp_id);
          list->push_back(static_cast<uint64_t>(tmp_id));
        }
      }
    }
  }
  if(OB_SUCCESS == ret)
  {
    TBSYS_LOG(INFO,"fetch collection list from ms[%s] ok, collection list count[%ld],dataversion[%ld]",to_cstring(ms),list->count(),data_version);
  }
  else
  {
    TBSYS_LOG(WARN, "fetch collection list from ms[%s] failed, ret[%d], version[%ld]",to_cstring(ms), ret, data_version);
  }
  return ret;
}
//add e
...
int ObRootRpcStub::statistic_signal_to_cs(const ObServer &cs, ObArray<uint64_t> &list)
{
  int ret = OB_SUCCESS;
  ...
  else if(OB_SUCCESS != (ret = serialization::encode_i64(msgbuf.get_data(),msgbuf.get_capacity(),msgbuf.get_position(),list.count())))
  {
    TBSYS_LOG(WARN,"failed to serialization list_count[%ld], err=%d",list.count(),ret);
  }
  else
  {
    for(int64_t i =0; i < list.count(); i++)
    {
      if(OB_SUCCESS != (ret = serialization::encode_i64(msgbuf.get_data(),msgbuf.get_capacity(),msgbuf.get_position(),list.at(i))))
      {
        TBSYS_LOG(WARN,"failed to serialization list[%ld]=%ld, err=%d",i,list.at(i),ret);
      }
    }
  }
  if(OB_SUCCESS != (ret = client_mgr_->post_request(cs,OB_START_GATHER_OPERATION,MY_VERSION,msgbuf)))
  {
    TBSYS_LOG(WARN,"failed to send request, ret=%d",ret);
  }
  else
  {
    TBSYS_LOG(INFO,"succeed to send request!");
  }
  return ret;
}
//add e
