...
ObCursorFetchAbsInto::ObCursorFetchAbsInto()
{
}
...
int ObCursorFetchAbsInto::add_variable(ObString &var)
{
	variables_.push_back(var);
	return OB_SUCCESS;
}

void ObCursorFetchAbsInto::reset()
{

}
void ObCursorFetchAbsInto::reuse()
{

}
int ObCursorFetchAbsInto::close()
{
	return OB_SUCCESS;
}

int ObCursorFetchAbsInto::get_row_desc(const common::ObRowDesc *&row_desc) const
{
	int ret = OB_SUCCESS;
...
int ObCursorFetchAbsInto::get_next_row(const common::ObRow *&row)
{
    int ret = OB_SUCCESS;
    if (NULL == child_op_)
    {
...
int ObCursorFetchAbsInto::open()
{
	TBSYS_LOG(INFO, "zz:ObCursorFetchAbsInto::open()");
	int ret = OB_SUCCESS;
	if (child_op_==NULL)
	{
		ret = OB_ERR_GEN_PLAN;
...
namespace oceanbase{
  namespace sql{
    REGISTER_PHY_OPERATOR(ObCursorFetchAbsInto, PHY_CURSOR_FETCH_ABS_INTO);
  }
}

int64_t ObCursorFetchAbsInto::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  databuff_printf(buf, buf_len, pos, "ObCursorFetchAbsInto (var list size=%ld)\n", variables_.count());
  return pos;
}
