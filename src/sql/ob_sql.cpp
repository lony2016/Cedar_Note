...
//add weixing [statistics build]20161215
#include "sql/ob_gather_statistics_stmt.h"
//add e
...
<<<<<<< HEAD
        ...
        if(ret == OB_SUCCESS)
        {
            ret = ObExistsLogicalOptimizer::optimize_exists(result_plan, result);
        }
        //add 20170629:e
        /* logical optimizer start, write by lxb start at 2016-12-14 */
        if(OB_SUCCESS == ret)
        {
          ret = pull_up_sublinks(&result_plan, logic_plan);

          // ret = ObOptimizer::optimizer(result_plan, result);
          ObBasicStmt *basic_stmt = logic_plan->get_main_stmt();
          if(basic_stmt!=NULL  
             && (basic_stmt->get_stmt_type()==ObBasicStmt::T_EXPLAIN 
                 || basic_stmt->get_stmt_type()==ObBasicStmt::T_SELECT))
          {
            ret = ObOptimizer::optimizer(result_plan, result);
          }
        }
...
<<<<<<< HEAD
      ...
      {
        ObPrivilege::TablePrivilege table_privilege;
        ObGatherStatisticsStmt *gather_statistics_stmt = dynamic_cast<ObGatherStatisticsStmt*>(stmt);
        if(OB_UNLIKELY(NULL == gather_statistics_stmt))
        {
          err = OB_ERR_UNEXPECTED;
          TBSYS_LOG(ERROR,"dynamic cast failed,err=%d", err);
        }
        ...
        {
          table_privilege.table_id_ = gather_statistics_stmt->get_statistics_table_id();
