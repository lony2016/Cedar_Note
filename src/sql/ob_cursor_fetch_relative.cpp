...
*
* @file ob_cursor_fetch_relative.cpp
...
ObFetchRelative::ObFetchRelative()
{
}
...
void ObFetchRelative::reset()
{
  ObSingleChildPhyOperator::reset();
}

void ObFetchRelative::reuse()
{
  ObSingleChildPhyOperator::reuse();
}

int ObFetchRelative::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  int ret = OB_SUCCESS;
...
int ObFetchRelative::get_next_row(const common::ObRow *&row)
{
  int ret = OB_SUCCESS;
  if (NULL == child_op_)
  {
  ...
  else
  {
   ...
   if(1 == cursor_op->get_is_opened())
   {
    if(fetch_count_<=0)
     ...
     {
       ret =  cursor_op->get_rela_row(is_next_,fetch_count_,row);
       if(ret == OB_ITER_END)
...
namespace oceanbase{
  namespace sql{
    REGISTER_PHY_OPERATOR(ObFetchRelative, PHY_CURSOR_FETCH_RELATIVE);
  }
}

int64_t ObFetchRelative::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
