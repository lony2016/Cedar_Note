...
*
* @file ob_cursor_fetch_last_into.cpp
...
ObCursorFetchLastInto::ObCursorFetchLastInto()
{
}
...
int ObCursorFetchLastInto::add_variable(ObString &var)
{
	variables_.push_back(var);
	return OB_SUCCESS;
}

void ObCursorFetchLastInto::reset()
{

}
void ObCursorFetchLastInto::reuse()
{

}
int ObCursorFetchLastInto::close()
{
	return OB_SUCCESS;
}

int ObCursorFetchLastInto::get_row_desc(const common::ObRowDesc *&row_desc) const
{
	int ret = OB_SUCCESS;
...
int ObCursorFetchLastInto::get_next_row(const common::ObRow *&row)
{
	int ret = OB_SUCCESS;
	if (NULL == child_op_)
	{
...
int ObCursorFetchLastInto::open()
{
	TBSYS_LOG(INFO, "zz:ObCursorFetchLastInto::open()");
	int ret = OB_SUCCESS;
	if (child_op_==NULL)
	{
		ret = OB_ERR_GEN_PLAN;
...
namespace oceanbase{
  namespace sql{
    REGISTER_PHY_OPERATOR(ObCursorFetchLastInto, PHY_CURSOR_FETCH_LAST_INTO);
  }
}

int64_t ObCursorFetchLastInto::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  databuff_printf(buf, buf_len, pos, "ObCursorFetchLastInto (var list size=%ld)\n", variables_.count());
  return pos;
}
