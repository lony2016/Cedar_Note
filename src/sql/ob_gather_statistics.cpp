 ...
 *Generating physical operato for collect statistico info  on no_pk columns
 */
...
int ObGatherStatistics::open()
{
  ...
  int64_t pos = 0;
  ObResultSet result;
  TBSYS_LOG(INFO,"weixing row_key info is %s",to_cstring(search_key_));
  ...
  if(OB_SUCCESS == ret)
  {
    for(int64_t i = 0; i < gather_info_.columns_list_.get_array_index(); i++)
    {
      ...
      if(pos >= 1023)
      {
        ...
        TBSYS_LOG(WARN, "gather buff overflow, ret = %d", ret);
      }
      else
      {
          ...
          {
            TBSYS_LOG(WARN, "direct execute sql =%.*s failed, ret=%d",insert_gather.length(),insert_gather.ptr(),ret);
          }
          ...
          else
          {
            ...
            TBSYS_LOG(INFO, "execute %.*s success", insert_gather.length(),insert_gather.ptr());
