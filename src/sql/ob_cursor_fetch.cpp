...
void ObCursorFetch::reset()
{
  ObSingleChildPhyOperator::reset();
}

void ObCursorFetch::reuse()
{
  ObSingleChildPhyOperator::reuse();
}

int ObCursorFetch::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  int ret = OB_SUCCESS;
 ...
 TBSYS_LOG(INFO, "ObCursorFetch::open()");
  ...
  if ((result_set = session->get_plan(cursor_name_)) == NULL
   ...
   else
   {
     ...
     TBSYS_LOG(INFO, "my result_set get prepared, ret=%d", ret);
...
int ObCursorFetch::get_next_row(const common::ObRow *&row)
{
	TBSYS_LOG(INFO, "ObCursorFetch::get_next_row");
    int ret = OB_SUCCESS;
    if (NULL == child_op_)
    {
    ...
    else
    {
      ...
      if(1 == cursor_op->get_is_opened())
      {
        ...
        if(ret == OB_ITER_END)
        {
        	my_phy_plan_->get_result_set()->set_message("fetch error: you have fetched the end");
        	TBSYS_LOG(USER_ERROR, "fetch error: you have fetched the end");
        }
      }
      else
      {
...
namespace oceanbase{
  namespace sql{
    REGISTER_PHY_OPERATOR(ObCursorFetch, PHY_CURSOR_FETCH);
  }
}

int64_t ObCursorFetch::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
