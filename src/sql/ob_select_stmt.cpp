...
ObSelectStmt::~ObSelectStmt()
{
  ...
  //add e
  select_items_.clear();
  //add by qx [fix memory leak] 20170801 :b
  for (int32_t i = 0; i < from_items_.size(); i++)
  {
    if (from_items_.at(i).from_item_rel_opt_ != NULL)
    {
      from_items_.at(i).from_item_rel_opt_->~ObOptimizerRelation();
    }
  }
  //add :e
  ...
  //add by dhc [query optimization] 20170509 : b
  ...
  //add by qx [fix memory leak] 20170727 :b
  ...
  //add by qx [query optimization] 20170509 : b
  //free rel_opt_list and table_id_statInfo_map_
...
int ObSelectStmt::remove_joined_table(uint64_t table_id)
{
  ...
  if(i==num)
  {
    TBSYS_LOG(WARN,"remove joined table error,table_id = %ld",table_id);
    ret = OB_ERROR;
  }
  return ret;
}
