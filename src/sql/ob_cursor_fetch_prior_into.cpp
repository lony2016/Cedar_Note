...
*
* @file ob_cursor_fetch_prior_into.cpp
...
ObCursorFetchPriorInto::ObCursorFetchPriorInto()
{
}
...
int ObCursorFetchPriorInto::add_variable(ObString &var)
{
	variables_.push_back(var);
	return OB_SUCCESS;
}

void ObCursorFetchPriorInto::reset()
{

}
void ObCursorFetchPriorInto::reuse()
{

}
int ObCursorFetchPriorInto::close()
{
	return OB_SUCCESS;
}

int ObCursorFetchPriorInto::get_row_desc(const common::ObRowDesc *&row_desc) const
{
	int ret = OB_SUCCESS;
...
int ObCursorFetchPriorInto::get_next_row(const common::ObRow *&row)
{
	int ret = OB_SUCCESS;
	if (NULL == child_op_)
	{
...
int ObCursorFetchPriorInto::open()
{
	TBSYS_LOG(INFO, "zz:ObCursorFetchPriorInto::open()");
	int ret = OB_SUCCESS;
	if (child_op_==NULL)
	{
		ret = OB_ERR_GEN_PLAN;
...
namespace oceanbase{
  namespace sql{
    REGISTER_PHY_OPERATOR(ObCursorFetchPriorInto, PHY_CURSOR_FETCH_PRIOR_INTO);
  }
}

int64_t ObCursorFetchPriorInto::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  databuff_printf(buf, buf_len, pos, "ObCursorFetchPriorInto (var list size=%ld)\n", variables_.count());
  return pos;
}
