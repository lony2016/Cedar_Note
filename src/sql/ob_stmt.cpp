...
<<<<<<< HEAD
  ...
  if(super_stmt != NULL)
  {
    common::ObVector<TableItem>& super_table_items = super_stmt->get_table_items();
    num = super_table_items.size();
    for (int32_t i = 0; i < num; i++)
    {
      TableItem& item = super_table_items[i];
      //modify slwang [exists related subquery] 20171101:b
      //if (table_name == item.table_name_ || table_name == item.alias_name_)
      if ((table_name == item.table_name_ && (item.alias_name_ == NULL)) || (table_name == item.alias_name_))
      //modify 20171101:e
      {
