...
//      }
    ...
    case T_OP_NAME_FIELD:
    {
      ...
      else
      {
        ...
        {
            ObRowDesc* table_hash = stmt->get_table_hash();
            int64_t index = table_hash->get_idx(column_item->table_id_, OB_INVALID_ID);
            if(OB_INVALID_INDEX == index)
            {
                ret = table_hash->add_column_desc(column_item->table_id_, OB_INVALID_ID);//添加相关子查询的表进子查询的table_hash_中
            }
        }
        //add 20170622:e
    ...
    case T_OP_CNN:
    {
      ...
      ret = resolve_expr(result_plan, stmt, node->children_[1], sql_expr, sub_expr2, expr_scope_type, true, super_stmt);
      //modify 20170622:e
      if (ret != OB_SUCCESS)
        break;

      //add by qx [join_without_pushdown_is_null/query_optimizer] 20170915:b
  ...
  if (node)
  {
    ...
    for (int32_t i = 0; i < node->num_child_; i++)
    {
      ...
      case T_JOIN_OP_TYPE_LIST:
        ...
        break;
        /*add e*/
        //add by qx [query optimizer] 20170730 :b
      case T_NO_QUERY_OPT:
        query_hint.no_query_opt_ = true;
